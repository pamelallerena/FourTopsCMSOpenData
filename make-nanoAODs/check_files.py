...
def main(input_dir):
    ...
    # Check file sizes
    any_file_less_than_2mb = False 

    for index, filepath in files.items():
        size_in_bytes = os.path.getsize(filepath)
        size_in_megabytes = size_in_bytes / (1024 * 1024)  # Convert bytes to megabytes

        if size_in_megabytes < 2:
           print("File with ID {} ({}) is less than 2MB ({:.2f} MB).".format(index, os.path.basename(filepath), size_in_megabytes))
           any_file_less_than_2mb = True
   
    if not any_file_less_than_2mb:
        print("None of the files is less than 2MB.")
